...
tabla_correlacion_ideologia_absoluto <- tryCatch(leer_resumen("18b2_correlacion_ideologia_viajes_intralatam_absoluto.csv"), error = function(e) NULL)
...
if (requireNamespace("xtable", quietly = TRUE) && !is.null(tabla_perfil_regional)) {
  print(xtable::xtable(fix_na_cuadro(tabla_perfil_regional),
                        caption = "Perfil regional de cada MANDATO individual: porcentaje de sus viajes a cada bloque de destino. Las categor\\'ias son mutuamente excluyentes y suman 100\\% por fila. Para mandatos iniciados antes de 1994 (ej. primer mandato de Menem, Fujimori, S\\'anchez de Lozada, entre otros), el a\\~no de inicio mostrado es 1994, ya que nuestra base solo tiene cobertura verificada desde esa fecha -el mandato real empez\\'o antes, pero solo se cuentan los viajes a partir de 1994.",
...
% anterior, pero contra la CANTIDAD ABSOLUTA de viajes intra-latinoamericanos
% en vez del porcentaje -para chequear si el patron de la version en \%
% se sostiene al mirar el volumen bruto de viajes.
<<tab-correlacion-ideologia-absoluto, echo=FALSE, results=tex>>=
if (requireNamespace("xtable", quietly = TRUE) && !is.null(tabla_correlacion_ideologia_absoluto)) {
  print(xtable::xtable(tabla_correlacion_ideologia_absoluto,
                        caption = "Correlaci\\'on de Pearson entre cada dimensi\\'on de ideolog\\'ia presidencial y la cantidad ABSOLUTA de viajes intra-latinoamericanos, por mandato-tramo (1994-2025).",
                        label = "tab:correlacion-ideologia-absoluto"),
        include.rownames = FALSE, caption.placement = "top",
        latex.environments = c("center"))
} else {
  cat("% Falta 18b2_correlacion_ideologia_viajes_intralatam_absoluto.csv o el paquete 'xtable'.\n")
}
@
...
% Figura NUEVA (2026-09-06, pedido del usuario): misma figura anterior, pero
% contra la CANTIDAD ABSOLUTA de viajes intra-latinoamericanos en vez del \%.
\begin{figure}[H]
  \centering
...
% Sudamerica con origen Y destino matcheados a la base de ideologia (1642
% viajes; el cruce diadico de esta subseccion, mas estricto que el de la
% seccion 4.4 -exige ideologia de AMBOS lados, origen y destino-, en la
% practica llega hasta 2024, no 2025: el ultimo viaje de 19a_homofilia_
% ideologica_diadico.csv es de 2024-12-14. Corregido 2026-09-06 -antes decia
% "1994-2025" en varios lugares de esta subseccion, lo cual no reflejaba la
% cobertura real de este cruce especifico).
...
if (requireNamespace("xtable", quietly = TRUE) && !is.null(tabla_indice_homofilia)) {
  print(xtable::xtable(tabla_indice_homofilia,
                        caption = "\\'Indice de homofilia ideol\\'ogica (viajes observados a la misma ideolog\\'ia / viajes esperados bajo independencia estad\\'istica), por bloque, 1994-2024. Un valor $>1$ indica m\\'as visitas ``propias'' de las esperadas por azar.",
...
\begin{figure}[H]
  ...
  \caption{Matriz de bloque ideol\'ogico del origen (fila) vs. bloque ideol\'ogico del destino (columna), \% de fila, todo el per\'iodo 1994-2024 en conjunto.}
