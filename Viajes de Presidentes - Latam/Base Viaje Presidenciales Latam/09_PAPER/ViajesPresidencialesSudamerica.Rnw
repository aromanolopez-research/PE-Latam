...
# de cada columna. Ecuador, Guyana, Surinam y Venezuela todavia no pasaron
# por esta verificacion y quedan marcados "Sin iniciar".
if (requireNamespace("xtable", quietly = TRUE) && !is.null(diferencias_colt)) {
  ...
  print(xtable::xtable(diferencias_colt,
                        caption = "Diferencias encontradas al cruzar y verificar la base contra la Base COLT (Diplometrics), por pa\\'is: viajes agregados que COLT no ten\\'ia, viajes con al menos un campo corregido, candidatos propios finalmente no incorporados, y filas que quedan pendientes de validar por falta de fuente independiente. Ecuador, Guyana, Surinam y Venezuela todav\\'ia no pasaron por esta verificaci\\'on.",
...
% Nota tecnica (2026-09-02, reemplaza la version anterior "vertical
% forzada"): con 8 columnas de region + Pais + Mandatario + Total viajes (11
% columnas en total) la tabla no entraba ni en vertical con margenes de 1cm
% y fuente tiny -quedaba desbordada y descentrada, como marco el usuario-.
% Se pasa a landscape real (paquete pdflscape) dentro de \begin{landscape},
% que da bastante mas ancho util (~24cm en A4 apaisado vs ~19cm en vertical
% con margen de 1cm) y deja que longtable centre la tabla como corresponde
% -pdflscape rota la PAGINA del PDF, no solo el contenido, asi que se lee
% bien girando el celular/monitor u hoja impresa-. Requiere tener pdflscape
% instalado (viene en cualquier instalacion completa de TeX Live/MiKTeX; si
% falta, instalarlo desde MiKTeX Console: Packages -> buscar "pdflscape" ->
% Install, o "tlmgr install pdflscape" en TeX Live).
\begin{landscape}
